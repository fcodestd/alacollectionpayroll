...
import { Search, ChevronRight, UserCircle, FileSignature } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";

export default function HarianRekapClient({ data }: { data: any[] }) {
    ...
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden min-h-[60vh]">
      <div className="p-6 border-b border-slate-100 bg-slate-50/50">
        <div className="relative max-w-md">
          ...
          <Input
            placeholder="Cari nama karyawan harian..."
        ...
        {filteredData.length > 0 ? (
          <div className="grid gap-1">
            {filteredData.map((emp) => (
              <div
                key={emp.id}
                className="flex items-center justify-between p-4 border border-transparent rounded-xl hover:bg-emerald-50/50 hover:border-emerald-100 transition-all group"
              >
                ...
                <Button
                  onClick={() =>
                    router.push(`/dashboard/rekap/harian/${emp.id}`)
                  }
                  className="bg-slate-900 hover:bg-emerald-600 text-white rounded-lg px-6"
                >
