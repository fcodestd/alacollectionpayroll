...
export default function JahitRekapClient({ data }: { data: any[] }) {
  ...
  // Filter client-side berdasarkan input pencarian
        ...
        {filteredData.length > 0 ? (
          <div className="grid gap-1">
            {filteredData.map((emp) => (
              <div
                key={emp.id}
                className="flex items-center justify-between p-4 border border-transparent rounded-xl hover:bg-blue-50/50 hover:border-blue-100 transition-all group"
              >
                ...
                <Button
                  onClick={() =>
                    router.push(`/dashboard/rekap/jahit/${emp.id}`)
                  }
                  className="bg-slate-900 hover:bg-blue-600 text-white rounded-lg px-6"
                >
